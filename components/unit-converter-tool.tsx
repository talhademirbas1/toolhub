...
const UNITS: Record<Category, { key: string; tr: string; en: string }[]> = {
  length: [
    { key: "mm", tr: "Milimetre", en: "Millimeter" },
    { key: "cm", tr: "Santimetre", en: "Centimeter" },
    { key: "m", tr: "Metre", en: "Meter" },
    { key: "km", tr: "Kilometre", en: "Kilometer" },
    { key: "inch", tr: "İnç", en: "Inch" },
    { key: "ft", tr: "Fit", en: "Feet" },
    { key: "mile", tr: "Mil", en: "Mile" },
  ],
  weight: [
    { key: "mg", tr: "Miligram", en: "Milligram" },
    { key: "g", tr: "Gram", en: "Gram" },
    { key: "kg", tr: "Kilogram", en: "Kilogram" },
    { key: "lb", tr: "Pound", en: "Pound" },
    { key: "oz", tr: "Ons", en: "Ounce" },
  ],
  temperature: [
    { key: "celsius", tr: "Celsius (°C)", en: "Celsius (°C)" },
    { key: "fahrenheit", tr: "Fahrenheit (°F)", en: "Fahrenheit (°F)" },
    { key: "kelvin", tr: "Kelvin (K)", en: "Kelvin (K)" },
  ],
};

// Metre cinsinden baz oran
const LENGTH_TO_M: Record<string, number> = {
  mm: 0.001, cm: 0.01, m: 1, km: 1000, inch: 0.0254, ft: 0.3048, mile: 1609.344,
};

// Gram cinsinden baz oran
...
export default function UnitConverterTool({ lang }: UnitConverterToolProps) {
  ...
  const getUnitLabel = (unitKey: string) => {
    ...
    return unit ? (lang === "tr" ? unit.tr : unit.en) : unitKey;
  };

  const categoryLabels: Record<Category, { tr: string; en: string }> = {
    length: { tr: "Uzunluk", en: "Length" },
      ...
      <CardHeader className="text-center space-y-3">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">
            {lang === "tr" ? "MyToolKit Hesaplama Aracı" : "MyToolKit Calculation Suite"}
          </p>
        </div>
        ...
        <div className="flex justify-center gap-1 bg-muted p-1 rounded-lg text-xs mt-2">
          {(Object.keys(UNITS) as Category[]).map((cat) => (
            ...
            >
              {lang === "tr" ? categoryLabels[cat].tr : categoryLabels[cat].en}
            </button>
          ))}
        </div>
      </CardHeader>
        ...
        <div className="grid grid-cols-1 md:grid-cols-[1fr,auto,1fr] gap-3 items-end">
          ...
          <div className="flex justify-center pb-2">
            <button
              ...
              title={lang === "tr" ? "Birimleri Değiştir" : "Swap Units"}
            >
