...
import { type Locale } from "@/i18n.config"; // YENİ EKLENDİ
...
const labels = {
  ...
  es: {
    ...
    about: "Acerca de",
    contact: "Contacto",
    nav: "Enlaces del sitio",
  }
} as const;

export function SiteFooter({ lang }: { lang: Locale }) {
  // Eğer sistem es, tr, en dışında bir dil gönderirse güvenli bir şekilde TR dönsün
    ...
    <footer className="border-t border-border/60 bg-zinc-100 dark:bg-background text-muted-foreground">
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-3 px-5 py-8 text-xs sm:flex-row sm:items-center sm:justify-between sm:px-8 lg:px-10">
        ...
        <nav aria-label={t.nav} className="flex flex-wrap gap-x-5 gap-y-2">
          <Link href={`/${currentLang}/about`} className="hover:text-foreground">
            {t.about}
          </Link>
          <Link href={`/${currentLang}/contact`} className="hover:text-foreground">
            {t.contact}
          </Link>
          <Link href={`/${currentLang}/privacy`} className="hover:text-foreground">
            {t.privacy}
          </Link>
        </nav>
      </div>
